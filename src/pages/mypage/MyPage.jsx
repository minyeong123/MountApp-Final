...
// ✅ 산 데이터 임포트 (경로를 반드시 프로젝트 구조에 맞게 수정하세요)
    ...
    <View className="bg-white border border-gray-100 rounded-2xl p-4 mb-3 shadow-sm">
        <View className="flex-row gap-3">
            <View className="w-[100px] h-[100px] rounded-xl bg-gray-100 overflow-hidden border border-gray-200">
                {record.mapImage ? (
                    <Image source={record.mapImage} className="w-full h-full" resizeMode="cover" />
                ) : (
...
export default function MyPage() {
    ...
    const [user, setUser] = useState({ name: "", email: "", userid: "", profileImage: null, stats: { point: 0, like: 0 } });
    ...
    const fetchUserInfo = async () => {
        try {
            ...
            const mappedHiking = (hikingRes.data || []).map((record, index) => {
                let foundTrail = null;
                Object.values(MOUNTAIN_DATA).forEach(trails => {
                    const match = trails.find(t => t.name === record.trailName);
                    if (match) foundTrail = match;
                });
                ...
                if (recordDateStr) {
                    ...
                    if (recordDateStr.includes(thisMonthFilter)) periodType = "THIS_MONTH";
                    else if (recordDateStr.includes(lastMonthFilter)) periodType = "LAST_MONTH";
                }

                return {
                    id: record.id || index,
                    mountainName: record.trailName || "이름 없는 코스",
                    date: recordDateStr ? recordDateStr.split('T')[0].replace(/-/g, '. ') : "등산 완료",
                    ...
                    mapImage: foundTrail ? foundTrail.imageSource : null,
                    period: periodType
                };
            });
        ...
        } catch (error) { console.error("마이페이지 데이터 조회 실패:", error); } finally { setLoading(false); }
    };

    useEffect(() => { fetchUserInfo(); }, []);

    const handlePostClick = (id, type) => { router.push(`/community/${id}`); };
    ...
    const handleSelectMountain = (name) => { setSelectedMountain(name); setDropdownOpen(false); setShowAllHikingRecords(false); };
    ...
    if (loading) return ( <View className="flex-1 justify-center items-center bg-white"><ActivityIndicator size="large" color="#15803d" /></View> );

    return (
        <View className="flex-1 bg-white">
            <View className="pt-5 pb-4 px-5 border-b border-gray-50 flex-row justify-between items-center bg-white"><Text className="text-xl font-black text-gray-900">마이페이지</Text></View>
            <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                <View className="px-5 py-6 flex-row items-center justify-between">
                    <View className="flex-row items-center gap-4">
                        <View className="w-16 h-16 rounded-full bg-gray-100 items-center justify-center overflow-hidden border border-gray-200">
                            {user.profileImage ? (<Image source={{ uri: `${BACKEND_URL}${user.profileImage}` }} className="w-full h-full" />) : (<UserIcon size={32} color="#9CA3AF" />)}
                        </View>
                        <View><Text className="text-lg font-bold text-gray-900">{user.name || "사용자"}</Text><Text className="text-xs text-gray-500">{user.email || "이메일 정보 없음"}</Text></View>
                    </View>
                    <TouchableOpacity onPress={() => router.push('/mypage/profile')} className="flex-row items-center px-3 py-2 border border-gray-200 rounded-full"><Text className="text-xs text-gray-600 mr-1">수정</Text><ChevronRight size={14} color="#9CA3AF" /></TouchableOpacity>
                </View>

                <View className="flex-row px-5 gap-3 mb-6">
                    <View className="flex-1 bg-gray-50 p-4 rounded-2xl flex-row justify-between items-center"><View><Text className="text-[10px] text-gray-400 font-bold mb-1">게시글/리뷰</Text><Text className="text-xl font-black text-gray-900">{user.stats.point}</Text></View><FileText size={20} color="#4B5563" /></View>
                    <View className="flex-1 bg-gray-50 p-4 rounded-2xl flex-row justify-between items-center"><View><Text className="text-[10px] text-gray-400 font-bold mb-1">좋아요</Text><Text className="text-xl font-black text-gray-900">{user.stats.like}</Text></View><Heart size={20} color="#4B5563" /></View>
                </View>
                ...
                <View className="px-5 mb-4">
                    ...
                    <View className="flex-row justify-between items-center mb-4">
                        <View className="flex-row gap-2">
                            {[{ id: 'ALL', label: '전체' }, { id: 'THIS_MONTH', label: '이번 달' }, { id: 'LAST_MONTH', label: '지난 달' }].map((tab) => (
                                <TouchableOpacity key={tab.id} onPress={() => setHikingTab(tab.id)} className={`px-4 py-2 rounded-full ${hikingTab === tab.id ? "bg-gray-900" : "bg-gray-100"}`}><Text className={`text-xs font-bold ${hikingTab === tab.id ? "text-white" : "text-gray-500"}`}>{tab.label}</Text></TouchableOpacity>
                            ))}
                        </View>
                        <View style={{ position: 'relative' }}>
                            <TouchableOpacity onPress={() => setDropdownOpen(!dropdownOpen)} className="flex-row items-center px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl"><Text className="text-xs text-gray-600 mr-1 font-semibold">{selectedMountain === 'ALL' ? '산별 보기' : selectedMountain}</Text><Text className="text-[10px] text-gray-500">{dropdownOpen ? '▲' : '▼'}</Text></TouchableOpacity>
                            {dropdownOpen && (
                                <View className="absolute right-0 top-12 bg-white border border-gray-200 rounded-xl shadow-lg z-50 w-32 overflow-hidden">
                                    <ScrollView style={{ maxHeight: 200 }}>
                                        {mountainList.map((name) => (
                                            <TouchableOpacity key={name} onPress={() => handleSelectMountain(name)} className={`px-4 py-3 border-b border-gray-100 ${selectedMountain === name ? 'bg-emerald-50' : 'bg-white'}`}><Text className={`text-xs ${selectedMountain === name ? 'text-emerald-700 font-bold' : 'text-gray-700'}`}>{name === 'ALL' ? '전체 산' : name}</Text></TouchableOpacity>
                                        ))}
                                    </ScrollView>
                                </View>
                            )}
                    ...
                    {visibleHikingRecords.length > 0 ? (
                        <>
                            {visibleHikingRecords.map((record) => (<HikingRecordCard key={record.id} record={record} />))}
                            {hasMoreHikingRecords && (
                                <TouchableOpacity onPress={() => setShowAllHikingRecords(prev => !prev)} className="w-full py-3 mt-1 mb-2 bg-gray-50 border border-gray-200 rounded-2xl items-center flex-row justify-center gap-1">
                                    <Text className="text-xs font-bold text-gray-500">{showAllHikingRecords ? '접기' : `더보기 (${filteredHikingRecords.length - HIKING_RECORDS_LIMIT}개 더)`}</Text>
                                    <Text className="text-[10px] text-gray-400">{showAllHikingRecords ? '▲' : '▼'}</Text>
                                </TouchableOpacity>
                            )}
                        </>
                    ) : (
                        <View className="items-center justify-center py-10 bg-gray-50 rounded-2xl"><Text className="text-gray-400 text-sm">기록된 등산 내역이 없습니다.</Text></View>
                    )}
                </View>
                ...
                <View className="px-5 mb-4">
                    ...
                    <View className="flex-row gap-2">{['ALL', 'POST', 'REVIEW'].map((tab) => (<TouchableOpacity key={tab} onPress={() => setActiveTab(tab)} className={`px-4 py-2 rounded-full ${activeTab === tab ? "bg-gray-900" : "bg-gray-100"}`}><Text className={`text-xs font-bold ${activeTab === tab ? "text-white" : "text-gray-500"}`}>{tab === 'ALL' ? '전체' : tab === 'POST' ? '게시글' : '리뷰'}</Text></TouchableOpacity>))}</View>
                </View>

                <View className="px-5 pb-10">
                    {filteredPosts.length > 0 ? (filteredPosts.map((post) => (<PostCard key={post.id} post={post} onClick={handlePostClick} />))) : (<View className="items-center justify-center py-20"><FileText size={40} color="#E5E7EB" /><Text className="text-gray-400 mt-2">작성된 내역이 없습니다.</Text></View>)}
                </View>

                <View className="px-5 mb-10 gap-3">
                    <TouchableOpacity onPress={handleLogout} className="w-full py-4 bg-white border border-gray-200 rounded-2xl items-center"><Text className="font-bold text-gray-700">로그아웃</Text></TouchableOpacity>
                    <TouchableOpacity onPress={() => Alert.alert("회원 탈퇴", "정말 탈퇴하시겠습니까?")} className="items-center py-2"><Text className="text-xs text-gray-400 underline">계정 탈퇴하기</Text></TouchableOpacity>
                </View>
            </ScrollView>
        </View>
    );
