...
// 🔥 에뮬레이터 연동: 안드로이드 에뮬레이터는 10.0.2.2, 실제 기기는 PC의 IP(예: 192.168.x.x)를 입력하세요.
const BACKEND_URL = "http://10.0.2.2:8082";

// 🔥 웹 버전의 로직을 그대로 가져온 이미지 경로 처리 함수
...
export default function Community() {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            // 웹과 동일하게 rating === 0은 게시글, rating > 0은 리뷰로 분리
                                ...
                                return (
                                    ...
                                    >
                                        <View className="h-28 bg-gray-200">
                                            {imagePath ? (
                                                <Image source={{ uri: getSafeImageUrl(imagePath) }} className="w-full h-full" />
                                            ) : (
                        ...
                        {filteredReviews.length > 0 ? (
                            filteredReviews.map((review, index) => {
                                ...
                                return (
                                    <TouchableOpacity
                                        key={reviewId || index}
                                        onPress={() => router.push({ pathname: "/community/[id]", params: { id: reviewId } })}
                                        className="w-[240px] bg-white rounded-2xl mr-3 overflow-hidden border border-gray-100 shadow-sm"
                                    >
                                        <View className="h-28 bg-gray-200">
                                            {imagePath ? (
