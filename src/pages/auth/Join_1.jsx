...
import { ChevronLeft, Eye, EyeOff, CheckCircle, XCircle } from "lucide-react-native";
...
export default function Join_1() {
    ...
    // 2. 입력 핸들러 (trim 추가로 공백 방지)
    const handleInputChange = (name, value) => {
        const cleanValue = name === "userid" || name === "password" ? value.trim() : value;
    ...
    const SERVER_URL = "http://10.0.2.2:8082"; // 에뮬레이터 기준 서버 주소
        ...
        <SafeAreaView className="flex-1 bg-white">
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled">
                ...
                <View className="px-8 mt-10">
                    <Text className="text-3xl font-black text-gray-900 leading-tight">환영합니다! 👋</Text>
                ...
                <View className="px-8 mt-8 space-y-6">
                    ...
                    {/* 활동명 */}
                    <View className="mb-4">
                        <Text className="text-sm font-bold text-gray-700 mb-2">활동명(별칭) <Text className="text-red-500">*</Text></Text>
                    ...
                    {/* 아이디 */}
                    <View className="mb-4">
                        <Text className="text-sm font-bold text-gray-700 mb-2">아이디 <Text className="text-red-500">*</Text></Text>
                        <View className="relative">
                            <TextInput
                    ...
                    {/* 비밀번호 */}
                    <View className="mb-4">
                        <Text className="text-sm font-bold text-gray-700 mb-2">비밀번호 <Text className="text-red-500">*</Text></Text>
                        <View className="relative justify-center">
                            <TextInput
                                ...
                                autoCapitalize="none"
                                className="w-full h-14 pl-5 pr-14 bg-gray-50 border border-gray-100 rounded-2xl text-gray-700"
                            />
                            <TouchableOpacity onPress={() => setShowPassword(!showPassword)} className="absolute right-5">
                                {showPassword ? <Eye size={22} color="#6366f1" /> : <EyeOff size={22} color="#9ca3af" />}
                            </TouchableOpacity>
                        </View>
                    </View>
                    ...
                    {/* 비밀번호 확인 */}
                    <View className="mb-8">
                        <Text className="text-sm font-bold text-gray-700 mb-2">비밀번호 확인 <Text className="text-red-500">*</Text></Text>
                        <View className="relative justify-center">
                            <TextInput
                                ...
                                autoCapitalize="none"
                                className={`w-full h-14 pl-5 pr-14 bg-gray-50 border rounded-2xl ${
                                    formData.passwordConfirm && formData.password !== formData.passwordConfirm
                                        ? 'border-red-400' : 'border-gray-100'
                                }`}
                            />
                            <TouchableOpacity onPress={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-5">
                                {showConfirmPassword ? <Eye size={22} color="#6366f1" /> : <EyeOff size={22} color="#9ca3af" />}
                            </TouchableOpacity>
                        </View>
                        {formData.passwordConfirm !== '' && formData.password !== formData.passwordConfirm && (
                ...
                {/* 하단 다음 버튼 (Link asChild 적용으로 내비게이션 에러 방지) */}
                <View className="px-8 py-10 mt-auto">
                    <Link
                        disabled={!isFormValid}
                        href={{
                            pathname: "/auth/join_2",
                            params: {
                    ...
                    >
                        <TouchableOpacity
                            ...
                            className={`w-full py-5 rounded-2xl items-center shadow-lg ${
                                isFormValid ? "bg-indigo-600 shadow-indigo-100" : "bg-gray-100"
                            }`}
                        >
