...
const safeSendMessage = (payload) => {
    try {
        sendMessage(payload, () => {}, (err) => console.log("워치 전송 실패:", err));
    } catch (error) {
        console.warn("⚠️ 워치 통신 모듈 사용 불가 환경", error);
    }
};
...
export default function MountainCourse({ id, initialTrail = null, onClose = null }) {
    ...
    const saveWorkoutSummary = async () => {
        try {
            const token = await AsyncStorage.getItem("jwtToken");
            if (!token) return;
            const avgHr = heartRateCountRef.current > 0 ? Math.round(heartRateSumRef.current / heartRateCountRef.current) : 0;
            ...
            await fetch(`${BACKEND_URL}/api/health/workout-summary`, {
                method: "POST",
                headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/json" },
    ...
    useEffect(() => {
                    ...
                    (location) => {
                        ...
                        if (lastLocationRef.current) {
                            ...
                            if (dist > 0.005) {
                                ...
                                setExtraMetrics(m => ({ ...m, altitude: currentAlt || m.altitude, calories: Math.floor(nextDistance * 75) }));
        ...
        if (isNavigating && !isPaused) startTracking();
        else {
            if (locationSubscriptionRef.current) {
                locationSubscriptionRef.current.remove();
                locationSubscriptionRef.current = null;
